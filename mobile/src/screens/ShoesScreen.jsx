...
export default function ShoesScreen({ navigation, route }) {
  ...
  // Starts already-expanded (no toggle animation) when arriving from Shop's
  // search bar via the `openSearch` param.
  const searchAnim = useRef(new Animated.Value(route?.params?.openSearch ? 1 : 0)).current;
